...
(function ($) {
    $.fn.inShare = function (options) {
        ...
        this.each(function () {
            ...
            obj.html('loading...');
            //构造体
            var ma = $("<div>").addClass("in-share-custom");
            var mb = $("<div>").addClass("in-share-box");
            var mc = $("<div>").addClass("in-share-cp"); //弹窗
            ...
            var ccimg = new Image();
            //构造开始
            if ($('link[href$="share22.css"]').length == 0) {//构造样式
                var css_href = options.domain + 'share22.css';
            ...
            $.each(options.hotjson["icons"], function (index, value) {
                $("<a>").attr("href", "javascript:void(0)").attr("title", value.title).attr("platform", value.other).attr("platformname", value.name).addClass("in-share-icon1 in-share-" + value.style).appendTo(ma);
            });
            var innerC = $('<div>').addClass('in-share-morebox');
            $.each(options.hotjson["bigicons"], function (index, value) {
                $("<a>").attr("href", "javascript:void(0)").attr("title", value.title).attr("platform", value.other).attr("platformname", value.name).addClass("in-share-icon2").append($("<em>").addClass(value.style)).append($("<span>").html(value.name)).appendTo(innerC);
            });
            innerC.wrapInner($("<div>").addClass("in-share-moreinnerbox"));
            obj.empty();
            ma.append($("<span>").addClass("in-share-count")).appendTo(obj);
            mb.append($("<div>").addClass("in-share-sbox").append($("<a>").attr("href", "javascript:void(0)").addClass("in-share-close").bind("click", function () {
                mb.hide();
                obj.css('zIndex', '');
            })).append($("<div>").attr("class", "in-share-ad")).append(innerC)).hide().appendTo(obj);
            ...
            obj.find(".in-share-custom a").not(".in-share-more").bind("click", function () {
                showshareboxIframe($(this).attr("platform"), $(this).attr("platformname"), surl, simg, stxt, options.popHeight, options.domain, siteid);
            });
            obj.find(".in-share-morebox a").bind("click", function () {
                showshareboxIframe($(this).attr("platform"), $(this).attr("platformname"), surl, simg, stxt, options.popHeight, options.domain, siteid);
            });
...
function showshareboxIframe(i, iname, surl, simg, stxt, sheight, sdomain, siteid) {
    var adpic = '';
    if (!havecheck) {
        ...
        if ($(".in-share-ad img").length) {
            for (i = 0; i < $(".in-share-ad img").length; i++) {
                ...
                adpic = adpic + ((i) ? ";" : "") + encodeURIComponent(yyimg.src) + "," + yyimg.width / yyimg.height;
                yyimg = null;
            }
    ...
    $("#in-share-iframe").attr("height", sheight - 24);
    ...
    $("#in-share-iframe").attr("src", sdomain + "share.aspx?siteid=" + siteid + "&snsname=" + iname + "&url=" + url + "&snsid=" + i + "&shareimg=" + simg + "&pagetitle=" + pagetitle + "&pagedesc=" + stxt);
